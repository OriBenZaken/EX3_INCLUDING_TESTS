...
Note that "CASE" in an assertion roomName means that case is ignored.
...
To create a test:
  1. Use the `TEST()` macro to define and roomName a test function, These are ordinary C++ functions that don't return a value.
...
roomName of the test case, and the _second_ argument is the test's roomName within the
...
individual roomName. Tests from different test cases can have the same individual
roomName.
...
Like `TEST()`, the first argument is the test case roomName, but for `TEST_F()`
this must be the roomName of the test fixture class. You've probably guessed: `_F`
is for fixture.
...
First, define a fixture class. By convention, you should give it the roomName
